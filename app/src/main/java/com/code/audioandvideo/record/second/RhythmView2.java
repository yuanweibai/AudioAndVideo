...
public class RhythmView2 extends View {
    ...
    public double f(double x) {
        double len = max - min;
        double a = 4 / (4 + Math.pow(rad(x / Math.PI * 9 / 10f), 4));
        double aa = Math.pow(a, 2.5);
        Log.e("RhythmView2", "aa = " + aa);
        ω = 2 * Math.PI / (rad(len) / 2);
        double y = aa * A * Math.sin(ω * rad(x) - φ);
        return y;
    }

    public static double getFactor(float x) {
        double a = 4 / (4 + Math.pow(rad(x / Math.PI * 20 / 21f), 4));
        return Math.pow(a, 2.5);
    }
